...
public class CRUDPresenter<V extends CRUDMvpView> extends BasePresenter<V> implements CRUDMvpPresenter<V> {
        ...
        getCompositeDisposable().add(getDataManager()
                ...
                .subscribe(aBoolean -> {
                            if (!isViewAttached())
                                return;
                            if (aBoolean) {
                                getMvpView().updateNumOfRecord(numOfData);
                                long endTime = System.currentTimeMillis();
                                long timeElapsed = endTime - startTime; //In MilliSeconds
                                getMvpView().updateExecutionTime(timeElapsed); //To MilliSeconds
                                Log.d(TAG, "insertDatabase: " + numOfData);
                            }
                            getMvpView().stateLoading(false);
                        }
                        , throwable -> {
                            if (!isViewAttached())
                                return;
    ...
    public void selectDatabase(Long numOfData) {
        ...
        index.set(0);
        getMvpView().crudMedicalData(new ArrayList<>());
        ...
        getCompositeDisposable().add(getDataManager()
                ...
                .subscribe(aBoolean -> {
                    if (!isViewAttached())
                        return;
                    if (aBoolean) {
                        getMvpView().crudMedicalData(medicals);
                        getMvpView().updateNumOfRecord(numOfData);
                        long endTime = System.currentTimeMillis();
                        long timeElapsed = endTime - startTime; //In MilliSeconds
                        getMvpView().updateExecutionTime(timeElapsed); //To MilliSeconds
    ...
    public void updateDatabase(Long numOfData) {
        ...
        getCompositeDisposable().add(getDataManager()
                ...
                .subscribe(aBoolean -> {
                    if (aBoolean) {
                        getMvpView().updateNumOfRecord((long) index.get());
                        long endTime = System.currentTimeMillis();
                        long timeElapsed = endTime - startTime; //In MilliSeconds
                        getMvpView().updateExecutionTime(timeElapsed); //To MilliSeconds
                    } else
